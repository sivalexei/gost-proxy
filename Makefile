...
SERVER_OBJ = $(BUILD_DIR)/server.o $(BUILD_DIR)/session.o $(CONFIG_OBJ) $(LOG_OBJ) $(TCP_HELPERS_OBJ) $(QUIC_LAYER_OBJ)
CLIENT_OBJ = $(BUILD_DIR)/client.o $(BUILD_DIR)/session.o $(CONFIG_OBJ) $(LOG_OBJ) $(SOCKS5_OBJ) $(QUIC_LAYER_OBJ)
...
$(CRYPTO_OBJ): $(CRYPTO_SRC) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -c $< -o $@
...
$(BUILD_DIR)/gost-client: $(CRYPTO_OBJ) $(CLIENT_OBJ) $(QUIC_LAYER_OBJ)
	$(CC) $^ -o $@ $(LDFLAGS)
