...
int quic_client_connect(quic_client_t *qc, const char *server_addr, uint16_t server_port,
    ...
    /* Проверяем ответ сервера: auth_tag = CMAC(session_id || server_nonce || session_nonce || conn_id)
    ...
    if (has_auth && r->payload[0] == 1) {
        ...
        log_info("QUIC: handshake OK (sid=%llu)",
                (unsigned long long)ntohll(r->session_id));
    } else {
        qc->active = 1;
        uint64_t sid_host = ntohll(*(uint64_t*)&r->session_id);
        memcpy(qc->session_id, &sid_host, 8);
...
ssize_t quic_server_recv(quic_server_t *qs, uint8_t *buf, size_t max_len,
    ...
    /* Подписываем eventfd для пробуждения (если задан) */
    if (qs->wfd >= 0) {
        struct epoll_event ev_w = { .events = EPOLLIN };
        ev_w.data.fd = qs->wfd;
        epoll_ctl(epfd, EPOLL_CTL_ADD, qs->wfd, &ev_w);
    }
    ssize_t n = 0;
    ...
    while (total_ms < timeout_ms) {
        ...
        struct epoll_event ev2[2];
        memset(ev2, 0, sizeof(ev2));
        int ret = epoll_wait(epfd, ev2, 2, chunk);
        ...
        if (ret == 0) { /* timeout on this chunk */ }
            ...
            /* Проверяем: это eventfd или UDP-сокет? */
            int got_eventfd = 0;
            for (int i = 0; i < ret; i++) {
            ...
            if (got_eventfd) continue;
            if (ev2[0].events & EPOLLIN) {
                close(epfd);
