...
static int tunnel_recv(uint8_t *out, size_t maxlen, int tmo, uint32_t ecid, uint32_t *ctr) {
    ...
    if (n > 0) {
        if (ntohl(pkt.magic)==GOST_PROXY_MAGIC && pkt.type==PKT_DATA && ntohl(pkt.conn_id)==ecid) {
            size_t dl;
            if (protocol_unpack_data(&pkt, out, &dl, NULL, &proxy_session, proxy_session.expanded_key,
...
static inline int socks5_auth_check(const uint8_t *buf, size_t len) {
    /* Проверяем пакет USER/PASS: ver(1) + ulen(1) + user + plen(1) + pass */
    if (len < 3 || buf[0] != 0x01) return 0;  /* only ver 1 supported */
    uint8_t ulen = buf[1];
    if (len < (size_t)(3 + ulen + 2)) return 0;  /* need at least plen + 1 byte */
    const char *user = (const char*)(buf + 2);
    uint8_t plen = buf[3 + ulen];
    const char *pass = (const char*)(buf + 4 + ulen);
    
    /* Если логин/пароль не заданы — пропускаем без аутентификации */
    if (socks5_user[0] == '\0' && socks5_pass[0] == '\0') {
        log_info("SOCKS5: no credentials set, allowing access");
        return 1;
    }
    
    /* Проверяем логин */
    if (ulen != strlen(socks5_user) || strncmp(user, socks5_user, ulen) != 0) {
        log_warn("SOCKS5: invalid username");
        return 0;
    }
    /* Проверяем пароль */
    if (plen != strlen(socks5_pass) || strncmp(pass, socks5_pass, plen) != 0) {
        log_warn("SOCKS5: invalid password");
        return 0;
    }
    
    log_info("SOCKS5: authentication successful");
    return 1;
}
...
static void* proxy_data_thread(void *arg) {
    ...
    while (socks5_running) {
        ...
        if (r>0&&(pf.revents&POLLIN)) {
            ...
            if (tunnel_send(buf,n,mcid,&client_send_ctr)!=0)break;
        }
        uint8_t resp[SOCKS5_BUF_SIZE];
        int rl=tunnel_recv(resp,sizeof(resp),50,mcid,&recv_ctr);
        if (rl>0) {
            size_t ts=0;
            while (ts<(size_t)rl) { ssize_t s=send(fd,resp+ts,rl-ts,MSG_NOSIGNAL); if (s<=0)goto cc; ts+=s; }
            while (socks5_running) {
                int el=tunnel_recv(resp,sizeof(resp),10,mcid,&recv_ctr); if (el<=0)break;
                size_t es=0;
...
static void* socks5_client_thread(void *arg) {
    ...
    /* Поддерживаем: 0x00 (no auth) или 0x02 (USER/PASS) */
    int need_auth = (socks5_user[0] != '\0' && socks5_pass[0] != '\0');
    uint8_t method = need_auth ? 0x02 : 0x00;
    
    for (int i=1; i<n; i++) {
        if (buf[i] == method) { method = buf[i]; break; }
    }
    
    uint8_t ar[]={0x05, method}; send(fd,ar,2,0);
    
    /* Шаг 2: Проверка USER/PASS */
    if (method == 0x02) {
        n=recv(fd,buf,sizeof(buf),0);
        if (n < 3 || !socks5_auth_check(buf, (size_t)n)) {
            uint8_t resp[]={0x01, 0x01}; send(fd,resp,2,0);  /* auth failure */
            close(fd); return NULL;
        }
        uint8_t resp[]={0x01, 0x00}; send(fd,resp,2,0);  /* auth success */
    }
    ...
    log_info("SOCKS5: calling tunnel_send, dlen=%d, cid=%u", dlen, mcid);
    int ts = tunnel_send(cd,dlen,mcid,&client_send_ctr);
...
static void* socks5_server_thread(void *arg) {
    ...
    while (socks5_running) {
        ...
        log_info("SOCKS5: accepted connection fd=%d", fd);
...
int socks5_start(uint16_t port, const char *user, const char *pass, const char *sip, uint16_t sport,
    ...
    if (user && user[0]) {
        memcpy(socks5_user, user, strnlen(user, sizeof(socks5_user) - 1));
        socks5_user[sizeof(socks5_user) - 1] = '\0';
    }
    if (pass && pass[0]) {
        memcpy(socks5_pass, pass, strnlen(pass, sizeof(socks5_pass) - 1));
        socks5_pass[sizeof(socks5_pass) - 1] = '\0';
    }
    memset(&proxy_session,0,sizeof(gost_session_t));
    proxy_session.session_id=sid;proxy_session.active=1;proxy_session.recv_counter=0;proxy_session.send_counter=0;
    proxy_session.recv_counter_window=0; memset(proxy_session.recv_bitmap,0,sizeof(proxy_session.recv_bitmap));
    log_info("SOCKS5: session_id set to %llu", (unsigned long long)sid);
    ...
    log_info("SOCKS5: listening on 0.0.0.0:%d", port);
    socks5_running=1;
    demux_running=1;
