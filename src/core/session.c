...
/* Deterministic 64→32 hash for padding length (both sides agree) */
static uint32_t padding_hash(uint64_t x) {
    x ^= x >> 33;
...
uint32_t protocol_insert_padding(uint8_t *p, uint32_t *dl, uint32_t padding_len, uint64_t sid) {
    ...
    *dl += padding_len;
    /* Fill padding with getrandom() */
    ssize_t nr = getrandom(p, padding_len, 0);
    if (nr < 0) {
        /* Fallback: xor with session_id to avoid all-zero padding */
        for (uint32_t i = 0; i < padding_len; i++)
            p[i] = (uint8_t)(sid ^ (sid >> 8) ^ i);
    }
    return padding_len;
}
...
int protocol_pack_data(gost_packet_t *pkt, uint64_t session_id, uint32_t conn_id,
    ...
    uint32_t pc;
    pc = __sync_fetch_and_add(counter, 2) + 2;
    ...
    /* Fill padding with getrandom() */
    {
        ssize_t nr = getrandom(pkt->payload+12, padding_len, 0);
        if (nr < 0) {
            for(uint32_t i=0;i<padding_len;i++)
                pkt->payload[12+i]=(uint8_t)(session_id^(session_id>>8)^i);
        }
    }
    ...
    log_debug("PACK_DATA: payload hdr mask offset 4-8");
    /* counter в [0..3] */
...
int protocol_unpack_data(const gost_packet_t *pkt, uint8_t *data, size_t *dl,
    uint32_t *oci, gost_session_t *sess, const uint8_t *ek, const uint8_t *nonce, uint32_t *ctr, uint8_t obf_dir) {
    if(oci) *oci = ntohl(pkt->conn_id);
    log_info("protocol_unpack_data: START");
    if(!pkt||!data||!dl||!sess||!ek||!nonce||!ctr){log_debug("UNPACK: param fail"); return -1;}
    ...
    hdr[10]=(h1>>8)&0xFF;hdr[11]=h1&0xFF;hdr[12]=(h2>>56)&0xFF;
    hdr[13]=(h2>>48)&0xFF;hdr[14]=(h2>>40)&0xFF;hdr[15]=(h2>>32)&0xFF;
    log_debug("UNPACK_DATA: payload hdr mask offset 4-8");
    deobfuscate_payload(deobf,sizeof(pkt->payload),hdr,obf_key);
    log_debug("protocol_unpack_data: payload deobfuscated");
    uint32_t pc=((uint32_t)deobf[0]<<24)|((uint32_t)deobf[1]<<16)|((uint32_t)deobf[2]<<8)|(uint32_t)deobf[3];
    /* Sliding window bitmap check — accept within [win-WIN..win], reject duplicates */
    {
        uint32_t wh = sess->recv_counter_window;  /* 0 = not initialized */
        if (wh == 0) {
            sess->recv_counter_window = pc;
            memset(sess->recv_bitmap, 0, sizeof(sess->recv_bitmap));
            sess->recv_bitmap[0] = 1;
        } else {
            uint32_t fwd;
            if (pc >= wh) fwd = pc - wh;
            else fwd = (uint32_t)(0x100000000u) - (wh - pc);
            if (fwd == 0 || fwd > COUNTER_WINDOW_SIZE) {
                log_info("UNPACK: counter dup/old pc=%u wh=%u fwd=%u",pc,wh,fwd);
                free(deobf); return -1;
            }
            int wi = (int)(fwd - 1) / 32;
            int bi = (int)(fwd - 1) % 32;
            if (sess->recv_bitmap[wi] & ((uint32_t)1 << bi)) {
                log_info("UNPACK: duplicate bitmap pc=%u",pc);
                free(deobf); return -1;
            }
            sess->recv_bitmap[wi] |= ((uint32_t)1 << bi);
            if (pc > wh) {
                uint32_t adv = pc - sess->recv_counter_window;
                if (adv >= COUNTER_WINDOW_SIZE) {
                    memset(sess->recv_bitmap, 0, sizeof(sess->recv_bitmap));
                    sess->recv_counter_window = pc;
                } else {
                    int sw = (int)(adv / 32);
                    int sb = (int)(adv % 32);
                    uint32_t tmp[COUNTER_WINDOW_BITS];
                    memset(tmp, 0, sizeof(tmp));
                    for (int i = sw; i < COUNTER_WINDOW_BITS; i++) {
                        if (sb == 0) tmp[i] = sess->recv_bitmap[i - sw];
                        else {
                            uint32_t hi = (i - sw + 1 >= 0) ? sess->recv_bitmap[i - sw + 1] : 0;
                            tmp[i] = (sess->recv_bitmap[i - sw] << sb) | (hi >> (32 - sb));
                        }
                    }
                    memcpy(sess->recv_bitmap, tmp, sizeof(sess->recv_bitmap));
                    sess->recv_bitmap[COUNTER_WINDOW_BITS - 1] |= ((uint32_t)1 << (sb == 0 ? 31 : sb - 1));
                    sess->recv_counter_window = pc;
                }
            }
        }
        *ctr = sess->recv_counter_window;
    }
...
int protocol_create_handshake(gost_packet_t *pkt, uint64_t session_id, const uint8_t *ek,
    ...
    (void)server_nonce; (void)session_nonce; log_info("HANDSHAKE CMAC: sid=%llu cid=%u", (unsigned long long)session_id, cid);
