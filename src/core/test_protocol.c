...
static void t_zero(void){
    ...
    if(protocol_unpack_data(&pk,d,&ulen,NULL,&sess,ek,nc,&ucn,0)!=0){FAIL(2,"unpack0");return;}
...
static void t_small(void){
    ...
    for(size_t l=1;l<=32;l++){
        ...
        if(protocol_unpack_data(&pk,out,&ulen,NULL,&sess,ek,nc,&ucn,0)!=0){printf("DEBUG t_small unpack l=%zu fail\n",l);failures++;continue;}
...
static void t_medium(void){
    ...
    for(size_t li=0;li<5;li++){
        ...
        if(protocol_unpack_data(&pk,out,&ulen,NULL,&sess,ek,nc,&ucn,0)!=0){failures++;continue;}
...
static void t_max(void){
    ...
    uint32_t ucn=0;
    if(protocol_unpack_data(&pk,out,&ulen,NULL,&sess,ek,nc,&ucn,0)!=0){FAIL(5,"unpack max");return;}
    ...
    uint32_t ucn=0;
    if(protocol_unpack_data(&pk,raw,&ulen,NULL,&sess,ek2,nc,&ucn,0)==0)FAIL(6,"wrong key");
...
static void t_bad_mac(void){
    ...
    if(protocol_unpack_data(&pk,raw,&ulen,NULL,&sess,ek,nc,&ucn,0)==0)FAIL(7,"bad mac");
...
static void t_replay(void){
    ...
    uint32_t ucn=0;
    if(protocol_unpack_data(&pk,raw,&ulen,NULL,&sess,ek,nc,&ucn,0)!=0)return;
    if(protocol_unpack_data(&pk,raw,&ulen,NULL,&sess,ek,nc,&ucn,0)==0)FAIL(8,"replay ok");
