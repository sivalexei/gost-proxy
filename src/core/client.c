...
*/
    ...
    session.active = 1;
    session.recv_counter = 0; session.send_counter = 0;
    session.recv_counter_window = 0; memset(session.recv_bitmap, 0, sizeof(session.recv_bitmap));
    ...
    printf("\n");
    uint16_t socks_port = cfg.socks5_port ? cfg.socks5_port : 1080;
    printf("========================================\n");
    ...
    if (socks5_start(socks_port, cfg.socks5_user, cfg.socks5_pass, cfg.server_ip, cfg.server_port, expanded_key,
                     session.nonce, session.session_id,
                     &quic_client, &session.recv_counter) != 0) {
    ...
    log_info("DISCONNECT: sid=%llu", (unsigned long long)session.session_id);
