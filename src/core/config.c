...
int config_load(gost_config_t *cfg, const char *path) {
    ...
    if (json_get_string(json, "socks5_user", tmp, sizeof(tmp)) == 0) {
        memcpy(cfg->socks5_user, tmp, strnlen(tmp, sizeof(cfg->socks5_user) - 1));
        cfg->socks5_user[sizeof(cfg->socks5_user) - 1] = '\0';
    }

    if (json_get_string(json, "socks5_pass", tmp, sizeof(tmp)) == 0) {
        memcpy(cfg->socks5_pass, tmp, strnlen(tmp, sizeof(cfg->socks5_pass) - 1));
        cfg->socks5_pass[sizeof(cfg->socks5_pass) - 1] = '\0';
    }
