...
/* Окно допустимых counter для sliding window replay protection */
#define COUNTER_WINDOW_SIZE 1024
#define COUNTER_WINDOW_BITS 32  /* 1024 / 32 = 32 uint32_t */
...
typedef struct {
    ...
    uint32_t recv_counter;    /* счётчик входящих пакетов (client->server) */
    uint32_t send_counter;    /* счётчик исходящих пакетов (server->client) */
    uint32_t recv_counter_window;  /* high watermark окна — 0 = окно не инициализировано */
    uint32_t recv_bitmap[COUNTER_WINDOW_BITS];  /* bitmap принятых counter в окне */
    int      active;
